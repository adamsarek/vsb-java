...
import pl1.utils.JavaFXRoutines;
...
public class CanvasManager {
	...
	public synchronized void add(Paintable paint) {
		subjects.add(paint);
		repaint();
	}

	public void remove(Paintable i) {
		subjects.remove(i);
		repaint();
	}

	public Paintable[] registeredSubjects() {
		return subjects.toArray(new Paintable[subjects.size()]);
	}
	...
	private static Node createControlPanel(Object thiz, double width, double height) {
		Pane result = JavaFXRoutines.loadFXML(thiz, "manipulation_panel.fxml");
	...
	public static class P_Application extends JavaFXApplication {
		...
		protected Region createRoot() {
			...
			root.setLeft(createControlPanel(CM.manipulationPanel = new ManipulationPanelController(), WIDTH, HEIGHT));
			return root;
		}
	...
	private class P_RepaintCommand implements Runnable {
		...
		public void run() {
			...
			try {
				getGraphics().fillRectangle(0, 0, getWidthInPoints(), getHeightInPoints(), colorOfBackground);

				for (Paintable subject : subjects) {
